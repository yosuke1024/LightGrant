...
- **Mitigation**:
  ...
  - **Browser identity binding (Sign in with Slack)**: Signed state and single-use nonce alone do not prove the *human completing GitHub OAuth* is the Slack user named in the state. An attacker could generate their own connect link and forward it to a victim, who would complete GitHub OAuth in their own browser — binding the victim's GitHub account to the attacker's Slack user. To close this, `/auth/github/start` first routes the browser through **"Sign in with Slack" (OpenID Connect)**. `/auth/slack/callback` refuses to proceed unless the OIDC-verified `team_id` / `user_id` equal the state's Slack workspace/user, and only then issues a one-time, `HttpOnly`, `SameSite=Lax` browser-binding cookie. The GitHub callback links accounts only when this cookie hashes to the value stored on the flow *and* `slack_verified_at` is set, so neither a forwarded start URL (verification fails as the wrong Slack user) nor a forwarded GitHub-authorize URL (victim's browser carries no binding cookie) can complete the link. The cookie is single-use: consuming the state clears its stored hash. See migration `0015_oauth_browser_binding.sql`.
